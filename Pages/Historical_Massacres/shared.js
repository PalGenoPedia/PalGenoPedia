...
/** Look up a key without falling back to the raw key string. */
function tRaw(key) {
    return (TRANSLATIONS[currentLang] || TRANSLATIONS.en)[key]
        || TRANSLATIONS.en[key]
        || null;
...
function applyI18n() {
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        // Keys in the "common." namespace belong to TranslationSystem
        // (header/footer chrome loaded from translations/*.json), not to
        // this page's local TRANSLATIONS map — leave them untouched.
        if (key.startsWith('common.')) return;

        const translation = tRaw(key);
        if (translation === null) return; // no page-local translation — leave as-is

        if (el.dataset.i18nAttr === 'placeholder') {
            el.placeholder = translation;
        } else {
            setTranslatedText(el, translation);
        }
    });
}
...
function renderIncidents() {
    ...
    container.innerHTML = filtered.slice(0, 100).map(i => {
        ...
        const hasFacility = facId && facilitiesData.some(f => f[cfg.facilityIdKey] === facId);
        const facSlug = encodeURIComponent(facId);
...
function openFacilityDetail(facilityId) {
    const cfg = window.PAGE_CONFIG;
    const fac = facilitiesData.find(f => f[cfg.facilityIdKey] === facilityId);
    if (!fac) { console.warn('Facility not found:', facilityId); return; }

    const relInc = incidentsData.filter(i => i[cfg.incidentFacKey] === facilityId);

    history.pushState({ facility: facilityId }, '', `#${cfg.urlPrefix}/${encodeURIComponent(facilityId)}`);

    document.querySelectorAll('.content-section').forEach(s => s.classList.remove('active'));
    document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
    document.getElementById(cfg.detailSectionId).classList.add('active');

    const facName = getField(fac, 'name', currentLang) || fac.name || facilityId;
    document.getElementById(cfg.breadcrumbId).textContent = facName;
    const slugEl = document.getElementById(cfg.permalinkSlugId);
    if (slugEl) slugEl.textContent = `#${cfg.urlPrefix}/${encodeURIComponent(facilityId)}`;
...
function openIncidentModal(incidentId, facilityId) {
    ...
    if (!inc) return;
    _currentFacilityId = facilityId || inc[cfg.incidentFacKey];

    const hash = `${cfg.urlPrefix}/${encodeURIComponent(_currentFacilityId)}/incident/${encodeURIComponent(incidentId)}`;
...
function closeIncidentModal() {
    ...
    if (_currentFacilityId) {
        const cfg = window.PAGE_CONFIG;
        history.pushState({ facility: _currentFacilityId }, '', `#${cfg.urlPrefix}/${encodeURIComponent(_currentFacilityId)}`);
    }
}
...
function routeFromHash() {
    ...
    // Pattern: <prefix>/FAC-000/incident/INC-001
